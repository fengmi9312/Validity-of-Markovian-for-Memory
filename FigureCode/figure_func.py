...
def figure_theories(_figure_theories_data):
    _fig = plt.figure(figsize = [12 * 1, 31 * 0.4])
    _axes = []
    ...
    xlabels = [[None, None, 'Time(d)'],['Time(d)', r'$R_0$'],[None, None, 'Time(d)', 'Time(d)', r'$\ln{\eta}$']]
    ylabels = [['Fraction(%)', None, 'Fraction(%)'],[None, 'Fraction(%)'],['Fraction(%)', None, 'Fraction(%)', None, r'$\varepsilon$']]
    ...
    plt.subplots_adjust(left=0.08, bottom=0.1, right=0.92, top=0.92, wspace=0.25, hspace=1)
    return _fig, _axes
...
def figure_forecasting(_figure_forecasting_data, _fitting_period_data):
    ...
    plt.text(0.52, 0.87, 'Markovian:', fontsize = 12, fontweight = 'bold', horizontalalignment='left', verticalalignment='center', transform=ax.transAxes, zorder = 100)
    plt.text(0.75, 0.87, 'non-Markovian:', fontsize = 12, fontweight = 'bold', horizontalalignment='left', verticalalignment='center', transform=ax.transAxes, zorder = 100)
    ax.add_artist(leg)
    ...
    plt.subplots_adjust(left=0.055, bottom=0.1, right=0.98, top=0.95, wspace=1, hspace=0)
    return _fig, _axes
...
def figure_prevention(_figure_prevention_data, _fitting_period_data):
    ...
    plt.subplots_adjust(left=0.065, bottom=0.08, right=0.98, top=0.92, wspace=1, hspace=0.5)
...
def si_figure_percentile(_si_figure_percentile_data):
    ...
    for i, percentile in enumerate([10, 90]):
        ...
        for j in range(5):
            ...
            plt.plot(res['log_ratio'], res['loss'], markers[j], color = colors[j], fillstyle='none', markersize = 3, label = labels[j])
        if i == 0:
            leg = plt.legend(fontsize = 11, ncol = 1, loc = 'upper right', frameon = False)
            for lh in leg.legendHandles: 
                lh.set_markersize(6)
    labels = [r'$\tilde{c} = 0.6$', None, None, None, None, r'$\tilde{c} = 0.4$', r'$\tilde{c} = 0.8$']        
    for i, percentile in enumerate([50, 5]):
        ...
        for j in [0, 5, 6]:
            ...
            plt.plot(res['log_ratio'], res['loss'], markers[j], color = colors[j], fillstyle='none', markersize = 3, label = labels[j])
        if i == 0:
            leg = plt.legend(fontsize = 11, loc = 'upper center', frameon = False)
            for lh in leg.legendHandles: 
                lh.set_markersize(6)
    ...
    for i, ax_arr in enumerate(_axes):
        for j, ax in enumerate(ax_arr):
            set_ax_format(ax, xlabels[i][j], ylabels[i][j], 14, 14, 16, spinetickwidth, spinewidth,
                          xtick_amounts[i][j], ytick_amounts[i][j], xlims[i][j], ylims[i][j], frame_types[i][j], 
                          titles[i][j], idx_titles[i][j], offset, _xtick_int = xtick_ints[i][j], _show_prop = show_props[i][j], _y_ext = y_exts[i][j], _set_lims = set_lims[i][j])
    plt.subplots_adjust(left=0.08, bottom=0.1, right=0.92, top=0.92, wspace=0.25, hspace=0.3)
    return _fig, _axes
...
def si_figure_markovian_time(_si_figure_markovian_time_data):
    ...
    set_ax_format(ax, r'$\ln{\alpha_{\mathrm{gen}}}$ ($\ln{\alpha_{\mathrm{rem}}}$)',r'$\hat{T}_{\mathrm{gen}}$ ($\hat{T}_{\mathrm{rem}})$', 14, 14, 16, spinetickwidth, spinewidth,
                  2, 2, (-0.3, 1.3), (5, 10), 2, 
                  None, None, offset, _xtick_int = False, _show_prop = False, _y_ext = True, _set_lims = True)
    plt.subplots_adjust(left=0.12, bottom=0.12, right=0.92, top=0.92, wspace=0.25, hspace=0.3)
...
def si_figure_error_percentile(_si_figure_error_percentile_data):
    ...
    for i, ax_arr in enumerate(_axes):
        for j, ax in enumerate(ax_arr):
            set_ax_format(ax, xlabels[i][j], ylabels[i][j], 18, 18, 18, spinetickwidth, spinewidth,
                          xtick_amounts[i][j], ytick_amounts[i][j], xlims[i][j], ylims[i][j], frame_types[i][j], 
                          titles[i][j], idx_titles[i][j], offset, _xtick_int = xtick_ints[i][j], _show_prop = show_props[i][j], _y_ext = y_exts[i][j], _set_lims = set_lims[i][j])
    plt.subplots_adjust(left=0.08, bottom=0.13, right=0.92, top=0.92, wspace=0.25, hspace=0.3)
    return _fig, _axes
