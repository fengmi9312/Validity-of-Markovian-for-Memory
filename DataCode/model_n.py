...
###############################################################################    
    ...
    def get_vac_x_prdt(self, vac_alloc, vac_freq, interval, target = 'c'):
        c_time_int = self.__current_time_int
        for i in range(vac_freq):
            self.add_vaccine(vac_alloc / vac_freq)
            for j in range(interval):
                self.spread_once()
        x = self.__get_x_from_c(self.solve_prdt().x, target)
        self.return_back(c_time_int)
        return x
...
def get_general_v_data(_idx, _spreading_survivals, _fitting_spreading_params, spreading_len,
               _i_in_data, _k, _amount_seeds, 
               simu_structure_params, calc_params, 
               daily_vac_amount, vac_times = 7, vac_duration = 24, occur_length = 4000):
...
def get_optimal_v_data(_idx, _simu_spreading_params, _fitting_spreading_params,
                       _i_in_data, _k, _amount_seeds, 
                       simu_structure_params, calc_params, 
                       daily_vac_amount, vac_times = 1, vac_duration = 24, consequent_step = 24 * 90, occur_length = 4000):
    ...
    for i, way in enumerate(['exponent', 'weibull']):
        ...
        for target in targets:
            ...
            simu_once.set_generator_seed(_idx)
            simu_once.set_spreading_func('weibull', 'weibull')
            simu_once.set_total_spreading_params(_simu_spreading_params[:2],
                                                 _simu_spreading_params[2:])
            simu_once.set_amount_seeds(_amount_seeds)
            ...
            simu_data[way][target + '_vac'] = _vac_amount_alloc
            del simu_once
    return simu_data



def get_optimal_general_v_data(_idx, _spreading_survivals, _fitting_spreading_params,
                       _i_in_data, _k, _amount_seeds, 
                       simu_structure_params, calc_params, 
    ...
    for i, way in enumerate(['exponent', 'weibull']):
        ...
        for target in targets:
            ...
            simu_once.spread_to_end()
            simu_data[way][target].append(simu_once.get_x_amount(target) / simu_once.get_node_amount())
            simu_data[way][target + '_vac'] = _vac_res
            del simu_once
    return simu_data
