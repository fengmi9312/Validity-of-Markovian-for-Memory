...
rank = 0 # The rank should be limited to the range(50)
...
for i in range(2):
    writer = pd.ExcelWriter("../ExperimentalData/explain_data/data_"+ str(rank + p_num * i) + ".xlsx")
    writer_add = pd.ExcelWriter("../ExperimentalData/explain_data/data_curves_"+ str(rank + p_num * i) + ".xlsx")
    param_list = {}
    res_list = {}
    rem_res_list = {}
    for idx, alpha_inf_n in enumerate((2.5, 1.5, 3.5)):
        fitting_res = get_fitting_data(rank + p_num * i + 1, (alpha_inf_n, get_beta_inf(alpha_inf_n, alpha_rem, beta_rem, lambda_eff), alpha_rem, beta_rem), 
                                       k, amount_seeds, 
