...
def import_contacts(country):
    sheet_names_1 = pd.ExcelFile(folder_path + '/DataDependencies/contact_matrices_152_countries/MUestimates_all_locations_1.xlsx').sheet_names
    sheet_names_2 = pd.ExcelFile(folder_path + '/DataDependencies/contact_matrices_152_countries/MUestimates_all_locations_2.xlsx').sheet_names
    idx = 0
    ...
    if idx == 0:
        return None
    home = pd.read_excel(folder_path + '/DataDependencies/contact_matrices_152_countries/MUestimates_home_' + str(idx) + '.xlsx', 
                      sheet_name = country, header = None).values
    school = pd.read_excel(folder_path + '/DataDependencies/contact_matrices_152_countries/MUestimates_school_' + str(idx) + '.xlsx', 
                      sheet_name = country, header = None).values
    work = pd.read_excel(folder_path + '/DataDependencies/contact_matrices_152_countries/MUestimates_work_' + str(idx) + '.xlsx', 
                      sheet_name = country, header = None).values
    other_locations = pd.read_excel(folder_path + '/DataDependencies/contact_matrices_152_countries/MUestimates_other_locations_' + str(idx) + '.xlsx', 
                      sheet_name = country, header = None).values
    return  {'home': home,
             'school': school,
             'work': work ,
...
def import_populations(country):
    data = pd.read_excel(folder_path + '/DataDependencies/populations/WPP2019_POP_F07_1_POPULATION_BY_AGE_BOTH_SEXES.xlsx', sheet_name = 'ESTIMATES')
