...
alpha_inf = 3
mean_inf = 5    # Here, you can set the value of the average infection time for the Weibull infection time distribution
beta_inf = get_beta_from_weibull(alpha_inf, mean_inf)
alpha_rem = 2
mean_rem = 7    # Here, you can set the value of the average removal time for the Weibull removal time distribution
...
for inf_pow, rem_pow in group_param_list[rank]:
    ...
    steady_data = {}
    writer = pd.ExcelWriter("../ExperimentalDataTmp/fitting_data_weibull_"+"i"+str(mean_inf)+"r"+str(mean_rem)+"/data_vac_"+ file_mark + ".xlsx")
    if add_mark:
        writer_add = pd.ExcelWriter("../ExperimentalDataTmp/fitting_data_weibull_"+"i"+str(mean_inf)+"r"+str(mean_rem)+"/data_vac_curves_"+ file_mark + ".xlsx")
