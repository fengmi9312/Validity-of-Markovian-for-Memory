...
if __name__ == '__main__':
    ...
    for pair_item in group_param_list[rank]:
        ...
        beta_rem = get_beta_from_weibull(alpha_rem, mean_rem)
        if add_mark:
            writer_add = pd.ExcelWriter("../ExperimentalDataTmp/fitting_data_weibull_"+"i"+str(mean_inf)+"r"+str(mean_rem)+"/data_curves_"+ file_mark + ".xlsx")
        writer = pd.ExcelWriter("../ExperimentalDataTmp/fitting_data_weibull_"+"i"+str(mean_inf)+"r"+str(mean_rem)+"/data_"+ file_mark + ".xlsx")
