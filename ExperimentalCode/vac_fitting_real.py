...
if __name__ == '__main__':
    rank = 0 # this value must be 0, 1, 2, or 3
    '''
    import pandas as pd
    from mpi4py import MPI
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    '''          # These four lines of code ensure that the code runs on the server utilizing Slurm to allocate computational resources to each calculation.
    ...
    for i in range(10):
        ...
        fitting_res = get_fitting_with_real(i + 1, spreading_params, k, amount_seeds, 
                 simu_structure_params, calc_params, params_fitting, flevel)
        calc_res = get_v_with_real(i + 1, spreading_params, fitting_res['params'], 
